...
# * Hiçbir şey bilmiyor olsak bu şekilde sıralardık. İç güdüye dayalı ortalamaya göre sıralardık
df.sort_values("vote_average", ascending=False).head(20)

df["vote_count"].describe([0.10, 0.25, 0.50, 0.70, 0.80, 0.90, 0.95, 0.99]).T


df[df["vote_count"] > 400].sort_values(
    "vote_average", ascending=False).head(20)
...
# * Kendimizce bir standartlaştırma uygulayabiliriz
...
M = 2500  # gereken minimum oy sayısı
C = df['vote_average'].mean()  # genel kitlenin ortalaması
...
df["weighted_rating"] = weighted_rating(
    df["vote_average"], df["vote_count"], M, C)
...
def bayesian_average_rating(n, confidence=0.95): # "bar score" olarak da bilinir
    if sum(n) == 0:
        return 0
    K = len(n)
...
bayesian_average_rating(
    [34733, 4355, 4704, 6561, 13515, 26183, 87368, 273082, 600260, 1295351])

# baba (9,1)
bayesian_average_rating(
    [37128, 5879, 6268, 8419, 16603, 30016, 78538, 199430, 402518, 837905])

# baba2 (9)
bayesian_average_rating(
    [20469, 3892, 4347, 6210, 12657, 26349, 70845, 175492, 324898, 486342])

# karasovalye (9)
bayesian_average_rating(
    [30345, 7172, 8083, 11429, 23236, 49482, 137745, 354608, 649114, 1034843])

# deadpole
bayesian_average_rating(
    [10929, 4248, 5888, 9817, 21897, 59973, 153250, 256674, 197525, 183404])
